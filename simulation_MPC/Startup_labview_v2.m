...
trajectoryfile='AAshortStraight_constant_highspeed2.csv';

% Store trajectory on format for Labview in the following file
trajectoryLabview='AAshortStraight_constant_highspeed2.csv';
csvwrite(trajectoryLabview,[Xref,Yref,Psiref,Vref,t])
...
[Xref,Yref,Psiref,Vref,t]=Refgeneration({'x','y','t'},trajectoryfile);
...
% Constant Speed needed for lqr [m/s]
    v_init = Vref(1); 
    if abs(v_init-2.5)>1.7, disp('Warning: LQR lateral controller set for low/high speed'); end
